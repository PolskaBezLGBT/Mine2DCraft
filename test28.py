...
obs_img3 = load_texture("obsidian.png", (64, 64), (20, 0, 40), False) # Nowa tekstura dla Endu
...
# Dodatkowy pool dla Endu (możesz tu dodać np. smoka lub więcej endermanów)
                ...
                for e in enemies[:]:
                    if br.colliderect(pygame.Rect(e['x']-cam_x, e['y']-cam_y, 50, 50)):
                        ...
                        if e['hp'] <= 0:
                            ...
                            # SZANSE NA PORTALE
                            if world_count == 1 and not portal_spawned and rnd < 0.05: # 5% do Netheru
                                portals.append({'rect': pygame.Rect(e['x'], e['y'], 100, 100), 'target': 2})
                                portal_spawned = True
                            elif world_count == 2 and not portal_spawned and rnd < 0.01: # 1% do Endu
